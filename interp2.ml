...
and trycatch () = 
((keyword "Try") >> many' command)  +++ ((keyword "Catch") >> many' command) << (keyword "End") >|= 
(fun (cmdtry, cmdscatch) -> Trycatch ( cmdtry, cmdscatch))


and func () = 
(keyword "DefFun") >> (name << ws) +++ (  name  << ws) +++ ( many' command ) << (keyword "End") >|=
 (fun ((fname, arg) , coms)  -> DefFun ((fname , arg), coms) )
...
let rec run (p : prog) (st : stack) (e : env) (log : string list) :
  ...
  | DefFun ((fname,args), cmds)::rest -> 
 run rest st (put e fname (F_val (e, fname, args, cmds))) log

  
 | Call :: rest -> 
 (match st with 
  | v :: F_val (env, fname, args, cmds):: rest2 -> (match run cmds [] (put (put env fname (F_val (env, fname, args, cmds))) args v) log with
  | (loge, Ok (y :: _)) -> run rest (y :: rest2) e loge
     | (loge, Ok _) -> loge, StackError
     | error -> error)
  | _ :: _ ::rest3 -> log, TypeError
  | _ -> log, StackError
  ...
  (match st with 
  | I_val x::_ -> log, Userdefined x
  | [] ->  log, StackError
  | _ -> log, TypeError
  )

  | Trycatch (cmdstry, cmdscatch) :: rest -> 
  (match run cmdstry [] e log with 

  | loge , x -> if (to_int_result x = 0) then run (cmdstry @ rest) st e log else run (cmdscatch@rest) (I_val (to_int_result x) ::st) e loge 
